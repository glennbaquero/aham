...
use App\Http\Requests\LocationPositionPost;
use App\Location;

class LocationFetchController extends FetchController
{
	/**
    ...
    public function fetchItem($id = null)
    {
        ...
        if ($id) {
            $item = Location::withTrashed()->find($id);
        }
    ...
    public function fetchPositions(LocationPositionPost $request)
    {
        $result = Location::getPositionByAddress($request->input('address'));
